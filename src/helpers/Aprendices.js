...
const aprendizHelper = {
    ...
    esTelefonoId : async (Telefono, id) => {
        const telefono = await Aprendiz.findOne({Telefono})
        if(telefono && telefono._id.toString() !== id){
            throw new Error(`El numero de Telefono ${Telefono} ya existe`)
        }
    },
